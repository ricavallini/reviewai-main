  ...
  const saveMarketplaceConfig = async (marketplaceId: string) => {
    ...
    try {
      if (marketplaceId === 'mercadolivre') {
        ...
        await mercadoLivre.connect(marketplace.credentials.apiKey);
      ...
      if (marketplaceId === 'mercadolivre') {
        ...
        setMarketplaces(prev => prev.map(marketplace => 
          marketplace.id === marketplaceId 
            ? { 
                ...marketplace, 
                status: mercadoLivre.isConnected ? 'connected' : 'error',
                lastSync: mercadoLivre.isConnected ? new Date() : marketplace.lastSync
              }
            : marketplace
        ));
      } else {
